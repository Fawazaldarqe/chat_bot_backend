...
const Patiant =Type.Object({
    ...
    const GetReservations2Query = Type.Object({
        sriose_case: Type.Optional(Type.String()),
    });
    ...
    export default async function (server: FastifyInstance) {
        server.route({
            ...
            handler: async (request, reply) => {
                ...
                if (!query.sriose_case) return reservations;
            	...
            	console.log(JSON.stringify(fuse.search(query.sriose_case)));
    
                const result: any[] = fuse.search(query.sriose_case).map((r) => r.item);
                return result;
